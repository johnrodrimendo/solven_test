...
import com.affirm.security.model.EntityExtranetRealmImpl;
...
@Configuration
@ComponentScan(value = {
        "com.affirm.entityExt.controller",
        "com.affirm.entityExt.service.impl",
        "com.affirm.entityExt.dao.impl"})
public class SpringEntityExtConfiguration extends WebMvcConfigurerAdapter {

    private static Logger logger = Logger.getLogger(SpringEntityExtConfiguration.class);
...
//    UserDAO userDao;
    ...
    @PostConstruct
    public void init() {
        System.setProperty("application", Application.ENTITY_EXT.name());
    }
    ...
    @Bean
    public EntityExtranetRealmImpl entityExtranetRealm() {
        EntityExtranetRealmImpl realm = new EntityExtranetRealmImpl();
        realm.setCredentialsMatcher((authenticationToken, authenticationInfo) -> true);
        realm.setDataSource(dataSource);
        realm.setPermissionsLookupEnabled(true);
        realm.setUserRolesQuery("select * from security.get_entity_roles(?)");
        realm.setPermissionsQuery("select * from security.get_permission(?)");
        return realm;
    }
    ...
    public Map<String, String> chainDefinitionFilterExtranetClient() {
        ...
        chain.put("/extranetEntityResetPassword/**", "anon");
        chain.put("/extranetEntityReset/**", "anon");
    ...
    public ShiroFilterFactoryBean shiroFilter() throws URISyntaxException {
        ...
        MultiSessionAuthenticationFilter entityFilter = new MultiSessionAuthenticationFilter();
        entityFilter.setPermitedUserClass(LoggedUserEntity.class);
        entityFilter.setLoginUrl("/login");
        entityFilter.setSuccessUrl("/");
    ...
    public DefaultWebSecurityManager securityManagerExtranetClient() throws URISyntaxException {
        ...
        realms.add(entityExtranetRealm());
        ...
        dwsm.getSessionIdCookie().setName("X-Entity-Session");
