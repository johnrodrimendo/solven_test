...
import com.affirm.entityExt.models.form.RejectLoanApplicationForm;
...
@Controller("entityExtranetLoanDetailController")
public class EntityExtranetLoanDetailController {

    public static final String URL = "loan_detail";
    ...
    @RequestMapping(value = "/" + URL + "/{loanId}", method = RequestMethod.GET)
    @RequiresPermissionOr403(permissions = "loan:detail:view", type = RequiresPermissionOr403.Type.WEB)
    @ErrorControllerAnnotation(responseType = ErrorControllerAnnotation.ResponseType.HTML)
    public Object showDetailEvaluation(
            ModelMap model, Locale locale, HttpServletRequest request, @PathVariable("loanId") Integer loanId,
            @RequestParam(value = "tray",required = false) Integer tray
            ) throws Exception {

        LoanApplication loanApplication = loanApplicationDao.getLoanApplication(loanId,locale);
        model.addAttribute("productCategoryId",loanApplication.getProductCategoryId());
        ...
        return new ModelAndView("/entityExtranet/fragments/extranetDetailFragments :: detail");
    }
...
//            return AjaxResponse.errorMessage("no existe una oferta seleccionada a&uacute;n");
        ...
        List<Date> enableDates = selectedOffer != null ? (loanApplication.getProductCategoryId() != null && loanApplication.getProductCategoryId().equals(ProductCategory.CUENTA_BANCARIA) ? new ArrayList<>() : question50Service.getScheduleEnablesDates(selectedOffer.getEntityProductParam())) : new ArrayList<>();
        ...
        RejectLoanApplicationForm rejectLoanApplicationForm = new RejectLoanApplicationForm();
        model.addAttribute("rejectLoanApplicationForm", rejectLoanApplicationForm);
...
//                    else if(aztecaPreApprovedBase.getIdCampania() == AztecaPreApprovedBase.NO_BANKED_CAMP) model.addAttribute("campaign", "No Bancarizados");
        ...
        identityValidationPainter.setLoanApplicationId(loanApplication.getId());
        ...
        RekognitionProResult rekognitionProResult = securityDAO.getRekognitionProResult(loanId);
        if(rekognitionProResult != null){
            identityValidationPainter.setRekognitionProResult(rekognitionProResult.getResponse());
        }

        RekognitionReniecResult rekognitionReniecResult = securityDAO.getRekognitionReniecResult(loanId);
        if(rekognitionReniecResult != null){
            identityValidationPainter.setRekognitionReniecData(rekognitionReniecResult.getResponse());
        }
       ...
       // borrar esto
        //summaryLoanExtranetPainter.getOffers().get(0).getProduct();

        // Get the status of the contract email for alfin
        if(loanApplication.getEntityId() != null && loanApplication.getEntityId() == Entity.AZTECA){
            List<PersonInteraction> personInteractions = interactionDAO.getPersonInteractionsByLoanApplication(loanApplication.getPersonId(), loanApplication.getId(), Configuration.getDefaultLocale());
            int[] interactions = personInteractions != null ?
                    personInteractions
                            .stream()
                            .filter(pi -> loanApplication.getId().equals(pi.getLoanApplicationId()))
                            .filter(pi -> pi.getInteractionProvider() != null && pi.getInteractionProvider().getId() == InteractionProvider.AWS)
                            .filter(pi -> pi.getInteractionContent().getId() == InteractionContent.AZTECA_APROBADO_ONLINE || pi.getInteractionContent().getId() == InteractionContent.AZTECA_CUENTA_SOLICITUD_APROBADA)
                            .limit(1)
            ...
            if(interactions.length > 0){
                Map<String, Date> emailEventMap = awsElasticSearchClient.getEmailEventsByPersonInteractionIds(interactions);
                String contractEmailStatus = "";
                contractEmailStatus += "Enviado - " + (emailEventMap.containsKey("Envio") ? utilService.datetimeShortFormat(emailEventMap.get("Envio")) + " - <i class=\"fa fa-check\" style=\"color: greenyellow\"></i> <br>"  : "");
                contractEmailStatus += "Entregado - " + (emailEventMap.containsKey("Entrega") ? utilService.datetimeShortFormat(emailEventMap.get("Entrega")) + " - <i class=\"fa fa-check\" style=\"color: greenyellow\"></i> <br>"  : "");
                contractEmailStatus += "Abierto - " + (emailEventMap.containsKey("Abierto") ? utilService.datetimeShortFormat(emailEventMap.get("Abierto")) + " - <i class=\"fa fa-check\" style=\"color: greenyellow\"></i> <br>"  : "");
                model.addAttribute("contractEmailStatus", contractEmailStatus);
            }
        }
        ...
        return new ModelAndView("/entityExtranet/fragments/extranetDetailFragments :: summaryData");
    }
    ...
    public Object getApplicantLoanApplication(
        ...
        model.addAttribute("loanId",loanId);
        model.addAttribute("personId", loanApplication.getPersonId());
        applicantLoanExtranetPainter.setNavLatitude(loanApplication.getNavLatitude());
        applicantLoanExtranetPainter.setNavLongitude(loanApplication.getNavLongitude());
        if(loanApplication != null){
            ...
            if(person != null){
                ...
                applicantLoanExtranetPainter.setFatca(fatcaData);
                ...
                if(disagregatedAddress != null){
                    ...
                    form.setLatitude(personContactInformation != null ? personContactInformation.getAddressLatitude() : null);
                    form.setLongitude(personContactInformation != null ? personContactInformation.getAddressLongitude() : null);
        ...
        return new ModelAndView("/entityExtranet/fragments/extranetDetailFragments :: applicantData");
    }
    ...
    public Object getIncomesLoanApplication(
        ...
        return new ModelAndView("/entityExtranet/fragments/extranetDetailFragments :: incomesData");
    }
    ...
    public Object updateAddress(
        ...
        if(loanId != null){
            LoanApplication loanApplication = loanApplicationDao.getLoanApplication(loanId, locale);
            if(personId != loanApplication.getPersonId()) throw new RuntimeException("Invalid PersonId");
        }
    ...
    public Object getPhoneVerification(
        ...
        return new ModelAndView("/entityExtranet/fragments/extranetDetailFragments :: phoneVerificationData");
    }
    ...
    public Object getAddressVerification(
        ...
        String postalCode = null;
        if(disagregatedAddress != null){
            if(disagregatedAddress.getTipoVia() != null) form.setRoadType(disagregatedAddress.getTipoVia().toString());
            if(disagregatedAddress.getTipoZona() != null) form.setZoneType(disagregatedAddress.getTipoZona().toString());
        }
        if (disagregatedAddress != null && disagregatedAddress.getUbigeo() != null && person.getDocumentType().getCountryId() == CountryParam.COUNTRY_PERU ) {
            Ubigeo ubigeo = catalogService.getUbigeo(disagregatedAddress.getUbigeo());
            department = ubigeo.getDepartment().getId();
        ...
        addressVerificationLoanExtranetPainter.setLongitude(personContactInformation != null ? personContactInformation.getAddressLongitude() : null);
        ...
        model.addAttribute("person",person);
        return new ModelAndView("/entityExtranet/fragments/extranetDetailFragments :: addressVerificationData");
    }
    ...
    public Object getRequestData(
        ...
        return new ModelAndView("/entityExtranet/fragments/extranetDetailFragments :: request");
    }
    ...
    public Object loadLoanApplicationAccordion(
            ...
            @PathVariable("accordion") String accordion,
            @RequestParam(value = "tray",required = false) Integer tray
            ) throws Exception {
        ...
        switch (accordion) {
            case "general":{
                ...
                fragmentToRender = "/entityExtranet/fragments/extranetDetailFragments :: generalInformation";
                break;
            }
            case "evaluation":{
                fragmentToRender = "/entityExtranet/fragments/extranetDetailFragments :: evaluationResult";
                List<LoanApplication> loans = loanApplicationDao.getLoanApplicationsByPerson(locale, personId, LoanApplication.class);
                if(loans != null){
                    LoanApplication fullLoan = loans.stream().filter(l -> l.getId().equals(loanApplication.getId())).findFirst().orElse(null);
                    if(fullLoan != null){
                        loanApplication.setRci(fullLoan.getRci());
                    }
                }
            ...
            case "documentation": {
                ...
                fragmentToRender = "/entityExtranet/fragments/extranetDetailFragments :: documentation";
            ...
            case "identity_validation": {
                ...
                DocumentationExtractedDataFromDatalabels documentationExtractedDataFromDatalabels = null;
                RekognitionProResult rekognitionProResult = securityDAO.getRekognitionProResult(loanId);
                if(rekognitionProResult != null){
                    identityValidationPainter.setRekognitionProResult(rekognitionProResult.getResponse());
                    if(rekognitionProResult.getResponse() != null){
                        documentationExtractedDataFromDatalabels = new DocumentationExtractedDataFromDatalabels();
                        documentationExtractedDataFromDatalabels.setDocumentNumber(rekognitionProResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_DOCUMENT_NUMBER_FRONT)).findFirst().orElse(null).getValue());
                        documentationExtractedDataFromDatalabels.setDateOfBirth(rekognitionProResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_DATE_OF_BIRTH)).findFirst().orElse(null).getValue());
                        documentationExtractedDataFromDatalabels.setPersonName(rekognitionProResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_PERSON_NAME)).findFirst().orElse(null).getValue());
                        documentationExtractedDataFromDatalabels.setFirstSurname(rekognitionProResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_FIRST_SURNAME)).findFirst().orElse(null).getValue());
                        documentationExtractedDataFromDatalabels.setOtherSurnames(rekognitionProResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_LAST_SURNAME)).findFirst().orElse(null).getValue());
                        documentationExtractedDataFromDatalabels.setLocality(rekognitionProResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_LOCATION)).findFirst().orElse(null).getValue());
                        documentationExtractedDataFromDatalabels.setDocumentExpirationDate(rekognitionProResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_EXPIRATION_DOCUMENT_DATE)).findFirst().orElse(null).getValue());
                    }
                }

                RekognitionReniecResult rekognitionReniecResult = securityDAO.getRekognitionReniecResult(loanId);
                if(rekognitionReniecResult != null && rekognitionReniecResult.getStatus() != null && rekognitionReniecResult.getStatus().charValue() == RekognitionReniecResult.SUCCESS_STATUS){
                    identityValidationPainter.setRekognitionReniecData(rekognitionReniecResult.getResponse());
                    if(rekognitionReniecResult.getResponse() != null){
                        documentationExtractedDataFromDatalabels = new DocumentationExtractedDataFromDatalabels();
                        documentationExtractedDataFromDatalabels.setDocumentNumber(rekognitionReniecResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_DOCUMENT_NUMBER_FRONT)).findFirst().orElse(null).getValue());
                        documentationExtractedDataFromDatalabels.setDateOfBirth(rekognitionReniecResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_DATE_OF_BIRTH)).findFirst().orElse(null).getValue());
                        documentationExtractedDataFromDatalabels.setPersonName(rekognitionReniecResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_PERSON_NAME)).findFirst().orElse(null).getValue());
                        documentationExtractedDataFromDatalabels.setFirstSurname(rekognitionReniecResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_FIRST_SURNAME)).findFirst().orElse(null).getValue());
                        documentationExtractedDataFromDatalabels.setOtherSurnames(rekognitionReniecResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_LAST_SURNAME)).findFirst().orElse(null).getValue());
                        documentationExtractedDataFromDatalabels.setLocality(rekognitionReniecResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_LOCATION)).findFirst().orElse(null).getValue());
                        documentationExtractedDataFromDatalabels.setDocumentExpirationDate(rekognitionReniecResult.getResponse().getData().stream().filter(e-> e.getType().equalsIgnoreCase(EntityProductParamIdentityValidationParamsConfig.LABEL_KEY_EXPIRATION_DOCUMENT_DATE)).findFirst().orElse(null).getValue());
                    }
                }
                identityValidationPainter.setDocumentationData(documentationExtractedDataFromDatalabels);
                identityValidationPainter.setIdentityApprovalValidation(loanApplication.getApprovalValidations().stream().filter(a-> a.getApprovalValidationId() == ApprovalValidation.IDENTIDAD).findFirst().orElse(null));
                ...
                boolean hasIdentityValidationsResult = identityValidationPainter.getMatiResult() != null || identityValidationPainter.getRekognitionProResult() != null || identityValidationPainter.getLastRekognition() != null;
                boolean hasReniecResult = false;
                ...
                if (entityWsResultDetalle != null) {
                    ...
                    hasReniecResult = reniecDataResponse != null;
                    if(hasReniecResult) identityValidationPainter.setReniecData(reniecDataResponse.getData());
                }
                ...
                person = personDAO.getPerson(catalogService, locale, personId, false);
                model.addAttribute("person", person);
                model.addAttribute("hasIdentityValidationsResult", hasIdentityValidationsResult);
                model.addAttribute("hasReniecResult", hasReniecResult);

                fragmentToRender = "/entityExtranet/fragments/extranetDetailFragments :: identityValidation";
            ...
            case "bank_account": {
                ...
                fragmentToRender = "/entityExtranet/fragments/extranetDetailFragments :: bankAccount";
                break;
            }
            case "notes": {
                model.addAttribute("notes", loanApplicationDao.getLoanApplicationComments(loanApplication.getId(), Comment.COMMENT_EVALUATION));

                fragmentToRender = "/entityExtranet/fragments/extranetDetailFragments :: notes";
                break;
            }
            case "sbs": {
...
//                        model.addAttribute("personRcc", personDAO.getPersonRcc(personId));
                ...
                fragmentToRender = "/entityExtranet/fragments/extranetDetailFragments :: sbs";
            ...
            case "interaction": {
                ...
                fragmentToRender = "/entityExtranet/fragments/extranetDetailFragments :: interaction";
            ...
            case "fraud_alert": {
                ...
                fragmentToRender = "/entityExtranet/fragments/extranetDetailFragments :: fraudAlert";
                break;
            }
            default:
                break;
    ...
    public Object resendValidationEmail(
        ...
        String errorMessageInvalidPermission = errorMessageInvalidPermission(loanApplication,loggedUserEntityId);
        if(errorMessageInvalidPermission != null) return AjaxResponse.errorMessage(errorMessageInvalidPermission);
...
//                    ALWAYS SEND VERIFICATION EMAIL
        ...
        personInteraction.setInteractionContent(catalogService.getInteractionContent(loanApplication.getCountry().getId() == CountryParam.COUNTRY_PERU ? InteractionContent.PERU_PROCESS_FINISHED_EMAIL : InteractionContent.ARG_PROCESS_SEND_EMAIL_VERIFICATION, loanApplication.getCountry().getId()));
        ...
        if(loanApplication.getCountry().getId() == CountryParam.COUNTRY_PERU){
            Map<String,Object> extraParams = new HashMap<>();
            extraParams.put(LoanApplicationServiceImpl.SHOULD_VALIDATE_EMAIL_KEY, true);
            linkToRedirect = loanApplicationService.generateLoanApplicationLinkEntity(loanApplication,extraParams);
        }

        if(linkToRedirect == null) linkToRedirect = String.format("%s/email-verification/%s", baseUrl, CryptoUtil.encrypt(verificationToken.toString()));
    ...
    private List<Bank> getPermittedBanks(int loanApplicationId, HttpServletRequest request) throws Exception {
        ...
        LoanOffer selectedoffer = offers != null ? offers.stream().filter(o -> o.getSelected() != null && o.getSelected()).findFirst().orElse(null) : null;
        entityProductParams = selectedoffer != null ? catalogService.getEntityProductParam(selectedoffer.getEntityId(), selectedoffer.getProduct().getId()) : null;
...
//                }
                ...
                switch (countryId) {
                    case CountryParam.COUNTRY_PERU:
                        ...
                        if (pbai.getBankAccount() == null)  return AjaxResponse.errorMessage("Debe ingresar un número de cuenta bancaria");

                        String code = pbai.getBankAccount().substring(pbai.getBankAccount().length() - 10);

                        if (!value.contains(code)) {
                            return AjaxResponse.errorMessage(String.format("El código <mark>%s</mark> no se encuentra incluido en el CCI <strong>%s</strong>", code, value));
                        }
...
//                }
                ...
                if(value.isEmpty() && value != null){
                    return AjaxResponse.errorMessage("Está ingresando un valor vacío.");
                }
                ...
                if(personService.getCurrentOcupationalInformation(personId, locale) == null) personDAO.cleanOcupationalInformation(personId, PersonOcupationalInformation.PRINCIPAL);
                personDAO.updateFixedGrossIncomeDouble(personId, 1, Double.parseDouble(value));
                personDAO.validateOcupationalInformation(personId, false);
                personDAO.updateOcupationalInformationBoChanged(personId, 1, true);
                updateRCI(loanId);
                break;
...
//                }
                ...
                if(personService.getCurrentOcupationalInformation(personId, locale) == null) personDAO.cleanOcupationalInformation(personId, PersonOcupationalInformation.PRINCIPAL);
                personDAO.updateVariableGrossIncomeDouble(personId, 1, Double.parseDouble(value));
                personDAO.validateOcupationalInformation(personId, false);
                personDAO.updateOcupationalInformationBoChanged(personId, 1, true);
                updateRCI(loanId);
                break;
...
//                }
                ...
                if (field.equals("partnerName")) {
                    ...
                    personDAO.updateBirthday(person.getPartner().getId(), new SimpleDateFormat(Configuration.BACKOFFICE_FRONT_ONLY_DATE_FORMAT).parse(value));
                }
                break;
            case "pep":
                // Validate the permission
...
//                }
                ...
                personDAO.updateOrInsertPersonDisqualifierByPersonIdOnly(loanApplication.getPersonId(), PersonDisqualifier.FACTA,Boolean.parseBoolean(value), null);
                //If we need to update the personal informacion as well, uncomment next line
                //personDao.updatePersonBankAccountInformation(personId, null, null, value, null, null);
                break;
...
//                }
    ...
    public Object returnToContract(
        ...
        if(loanApplication.getSelectedEntityProductParameterId() == null) return AjaxResponse.errorMessage("No se ha seleccionado una oferta");

        EntityProductParams entityProductParams = catalogService.getEntityProductParamById(loanApplication.getSelectedEntityProductParameterId());

        if(entityProductParams.getEntityProductParamExtraConfiguration() != null && entityProductParams.getEntityProductParamExtraConfiguration().getCreditAfterApproval() != null && entityProductParams.getEntityProductParamExtraConfiguration().getCreditAfterApproval()){
            if(loanApplication.getCredit() == null || !loanApplication.getCredit()) return AjaxResponse.errorMessage("No puede retornar esta solicitud a contrato");
        }

        loanApplicationDao.boResetContract(loanApplicationId);
        loanApplicationDao.updateLoanApplicationStatus(loanApplicationId,LoanApplicationStatus.EVAL_APPROVED,null);
        return AjaxResponse.ok(null);
    }
    ...
    @ErrorControllerAnnotation(responseType = ErrorControllerAnnotation.ResponseType.JSON)
    public Object expire(Locale locale,@RequestParam("loanApplicationId") Integer loanApplicationId) throws Exception {
    ...
    public Object approveLoanApplication(
        ...
        if(!loanApplicationApprovalValidationService.loanHasAllValidations(loanApplication,entityProductParams))
        ...
        if(loanApplication.getSelectedEntityProductParameterId() != null && Arrays.asList(
                EntityProductParams.ENT_PROD_PARAM_AZTECA_ONLINE,
                EntityProductParams.ENT_PROD_PARAM_AZTECA_CUENTA_AHORRO
        ).contains(loanApplication.getSelectedEntityProductParameterId())){
            if(selectedOffer.getSignatureFullName() == null || selectedOffer.getSignatureFullName().isEmpty()) return AjaxResponse.errorMessage("La oferta no ha sido firmada");
        }
        ...
        String message = "Se ha aprobado la solicitud exitosamente.";

        if(loanApplication.getSelectedEntityProductParameterId() != null){
            entityProductParams = catalogService.getEntityProductParamById(loanApplication.getSelectedEntityProductParameterId());
            if(entityProductParams.getEntityProductParamExtraConfiguration() != null && entityProductParams.getEntityProductParamExtraConfiguration().getMessageInManualApproval() != null) message = entityProductParams.getEntityProductParamExtraConfiguration().getMessageInManualApproval();
        }



        return AjaxResponse.ok(message);
    }
...
//        emailBoService.sendRejectionMail(la, locale);
        ...
        emailCLService.sendRejectionMailEvaluation(loanApplication.getId(),locale);
//        emailBoService.sendRejectionMail(la, locale);
        if(loanApplication.getEntityId() != null && loanApplication.getEntityId() == Entity.AZTECA){
            loanApplicationDao.updateCurrentQuestion(loanApplicationId ,ProcessQuestion.Question.DISAPPROVE_EVALUATION.getId());
        }
    ...
    private FieldValidator getGrossIncomeValidator(Integer personId, Locale locale, String value) throws Exception {
        ...
        if (person.getDocumentType().getCountryId().equals(CountryParam.COUNTRY_COLOMBIA))
            ...
            return new DoubleFieldValidator(ValidatorUtil.GROSS_INCOME_DOUBLE, Double.parseDouble(value));
    }
    ...
    public Object registerContact(
        ...
        registerContactResult(loggedUserEntityId, loanApplicationId,contactRelationship,referralId,contactResultId);

        return AjaxResponse.ok(null);
    }

    @RequestMapping(value = URL +"/loan/{loanApplicationId}/registerContactResultAddress", method = RequestMethod.POST)
    @RequiresPermissionOr403(permissions = "loan:contactResultAddressVerification:register", type = RequiresPermissionOr403.Type.AJAX)
    @ErrorControllerAnnotation(responseType = ErrorControllerAnnotation.ResponseType.JSON)
    public Object registerContactAddressVerification(
            ModelMap model, Locale locale, HttpServletRequest request,
            @PathVariable("loanApplicationId") Integer loanApplicationId,
            @RequestParam(value = "contactRelationship", required = false) Integer contactRelationship,
            @RequestParam(value = "referralId", required = false) Integer referralId,
    ...
    private void registerContactResult(Integer entityUserId, Integer loanApplicationId,Integer contactRelationship,Integer referralId,Integer contactResultId) throws Exception {
        loanApplicationDao.registerTrackingActionContactPerson(loanApplicationId, null, contactResultId, null, null, true, null,entityUserId, referralId, null);
    ...
    public Object identityManualApproval(
            ModelMap model, Locale locale, HttpServletRequest request,
            @PathVariable("loanApplicationId") Integer loanApplicationId,
            @RequestParam(required = false,value = "processId") Integer processId,
        ...
        if(loanApplication != null){
            //if(type == "MATI") securityDAO.updateMatiStatus(processId,status != null && status ? MatiResult.MATI_STATUS_VERIFIED : MatiResult.MATI_STATUS_REJECTED);
        }
        loanApplicationApprovalValidationService.validateAndUpdate( loanApplicationId, ApprovalValidation.IDENTIDAD, status);
        if(status != null && status && loanApplication.getEntityId() != null && loanApplication.getEntityId() == Entity.AZTECA && loanApplication.getSelectedEntityId() != null && Arrays.asList(EntityProductParams.ENT_PROD_PARAM_AZTECA_ONLINE,EntityProductParams.ENT_PROD_PARAM_AZTECA_CUENTA_AHORRO).contains(loanApplication.getSelectedEntityId())){
           if(loanApplicationApprovalValidationService.loanHasAllValidations(loanApplication,entityProductParams)) this.loanApplicationService.sendLoanApplicationApprovalMail(loanApplication.getId(), loanApplication.getPersonId(), locale);
        }
        return AjaxResponse.ok(null);
    }
...
//                }
    ...
    public Object getWelcomeCallLoanApplication(
        ...
        return new ModelAndView("/entityExtranet/fragments/extranetDetailFragments :: welcomeCallData");
    }
...
//        loanApplicationDao.selectLoanOfferAnalyst(newOffer.getId(), backofficeService.getLoggedSysuser().getId());
    ...
    public Object getLoanOfferCreateModal(
        ...
        return new ModelAndView("/entityExtranet/fragments/extranetDetailFragments :: createLoanOfferModal");
    }
    ...
    public ResponseEntity<String> createLoanOffer(
        ...
        model.addAttribute("productCategoryId",loan.getProductCategoryId());
    ...
    public ResponseEntity<String> selectLoanOffer(
        ...
        if(Arrays.asList(Entity.AZTECA).contains(entityExtranetService.getLoggedUserEntity().getPrincipalEntity().getId()))  return AjaxResponse.errorMessage("No puede seleccionar esta oferta");
    ...
    private String errorMessageInvalidPermission(LoanApplication loanApplication, Integer entityUserId){
        String errorMessage = null;

        if(loanApplication.getAssignedEntityUserId() == null) errorMessage = "Falta por asignar un analista";
        else if(loanApplication.getAssignedEntityUserId().intValue() != entityUserId) errorMessage  = "La solicitud está asignada a otro analista.";

        return errorMessage;
    }

    @RequestMapping(value = URL +"/action/validateamount", method = RequestMethod.POST)
    @RequiresPermissionOr403(permissions = "loan:action:approve", type = RequiresPermissionOr403.Type.WEB)
    @ErrorControllerAnnotation(responseType = ErrorControllerAnnotation.ResponseType.JSON)
    public Object validateAmount(
        ...
        loanApplicationApprovalValidationService.validateAndUpdate(loanApplicationId, ApprovalValidation.LIMITE_DE_MONTO, true);

        return AjaxResponse.ok(null);
    }

    private void returnLoanToContractAndUpdateDueDate(LoanApplication loanApplication, LoanOffer selectedOffer) throws Exception {
        if(selectedOffer != null && Arrays.asList(EntityProductParams.ENT_PROD_PARAM_AZTECA_ONLINE).contains(selectedOffer.getEntityProductParameterId())){
            ...
            loanApplicationDao.updateLoanOfferFirstDueDate(selectedOffer.getId(), newDueDate);
            loanApplicationDao.boResetContract(loanApplication.getId());
            ...
            loanApplicationDao.updateEntityCustomData(loanApplication.getId(),
                    loanApplication.getEntityCustomData().put(LoanApplication.EntityCustomDataKeys.AZTECA_DUE_DATE_MODIFIED_AFTER_CONTRACT.getKey(), true));
        }
    }
}
