(function (global, factory) {
  ...
  ***************************************************************************** */
  ...
  function drawDetections(canvasArg, detections, printBoxOption, takeButtonElement, excludeBox, isMobile) {
      let matching = false;
      var detectionsArray = Array.isArray(detections) ? detections : [detections];
      if(printBoxOption) printBox(canvasArg, printBoxOption.boxPercentage, false, isMobile);
      let lastBox = null;
      detectionsArray.forEach(function (det) {
          ...
          lastBox = box;
          if(printBoxOption && lastBox){
              let boxProperties = getPropertiesFromBox(canvasArg, printBoxOption.boxPercentage, isMobile);
              if(lastBox._width < boxProperties._width && lastBox._width >= (boxProperties._width*.60)){
              ...
              printBox(canvasArg, printBoxOption.boxPercentage, matching, isMobile);
              if(takeButtonElement){
                  if(matching) takeButtonElement.style.display = 'block'
                  else takeButtonElement.style.display = 'none'
              }
          }
          if(!excludeBox) new DrawBox(box, { label: label }).draw(canvasArg)
      });
      return matching;
  }
  ...
  function getModelUris(uri, defaultModelName) {
      ...
      if (uri === '/') {
          uri = 'https://solven-public.s3.amazonaws.com/external/face-api/';
          return {
              modelBaseUri: '',
              manifestUri: "/" + defaultManifestFilename
          };
  ...
  function matchDimensions(input, reference, useMediaDimensions) {
    ...
    function printBox(input, boxPercentage, isMatching, isMobile) {
        if(input){
            let ctx = getContext2dOrThrow(input);
            let boxProperties = getPropertiesFromBox(input, boxPercentage, isMobile);
    ...
    function getPropertiesFromBox(input, boxPercentage, isMobile){
      let minValue = Math.min(input.width, input.height)
      let baseValueWidth = minValue*boxPercentage;
      let baseValueHeight = minValue*boxPercentage;
      if(isMobile){
          baseValueHeight = input.height*0.85;
          baseValueWidth = input.width*.7;
      }else{
          let value = transformFromCanvasDimensionsToPixels(input.height, input.clientHeight, baseValueHeight)
          if(value < 200){
              baseValueHeight = transformFromPixelsToCanvasDimensions(input.height, input.clientHeight, 200);
          }
      }
      let xAxisDifference = input.width-baseValueWidth;
      let yAxisDifference = input.height-baseValueHeight;
      return {
          _x : xAxisDifference/2,
          _y : yAxisDifference/2,
          _width : baseValueWidth,
          _height : baseValueHeight
      }
    }

    function transformFromCanvasDimensionsToPixels(totalDimension, totalPixel, value){
      return (value*totalPixel)/totalDimension
    }

    function transformFromPixelsToCanvasDimensions(totalDimension, totalPixel, value){
        return (value*totalDimension)/totalPixel
    }
  ...
  exports.transformFromCanvasDimensionsToPixels = transformFromCanvasDimensionsToPixels;
  exports.transformFromPixelsToCanvasDimensions = transformFromPixelsToCanvasDimensions;
  exports.getPropertiesFromBox = getPropertiesFromBox;
