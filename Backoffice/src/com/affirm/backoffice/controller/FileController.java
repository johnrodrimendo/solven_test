...
public class FileController {
    ...
    @RequiresPermissionOr403(permissions = "person:documentation:upload", type = RequiresPermissionOr403.Type.AJAX)
    @ResponseBody
    ...
    public ResponseEntity<String> validateAllUserFiles(
        ...
        try {
            ...
            userDao.validateAllUserFile(userId, laId);
    ...
    public ResponseEntity<String> requestMissingDocumentation(
        ...
        if(loanApplication != null && (loanApplication.getStatus().getId() == LoanApplicationStatus.WAITING_APPROVAL || loanApplication.getStatus().getId() == LoanApplicationStatus.APPROVED)){
            ...
            loanApplicationService.sendMissingDocumentation(userId, loanApplicationId, locale);
            return AjaxResponse.ok(null);
        }else{
    ...
    @RequiresPermissionOr403(permissions = "person:documentation:move", type = RequiresPermissionOr403.Type.AJAX)
    @ResponseBody
    ...
    @RequiresPermissionOr403(permissions = "person:documentation:rotate", type = RequiresPermissionOr403.Type.AJAX)
    @ResponseBody
    ...
    @RequiresPermissionOr403(permissions = "person:documentationTab:view", type = RequiresPermissionOr403.Type.AJAX)
    public ResponseEntity<?> downloadContract(@RequestParam("creditIds") Integer[] creditIds, HttpServletRequest request) {
        BinaryOutputWrapper output = new BinaryOutputWrapper();
        ...
        try {
            ...
            if (creditIds != null) {
                for (Integer creditId : creditIds) {
                    ...
                    List<LoanApplicationUserFiles> userFilesObjectList = personDAO.getUserFiles(credit.getPersonId(), locale);
                    ...
                    List<UserFile> userFiles = userService.getUserFileByType(credit.getUserFilesObjectList().getUserFileList(), UserFileType.CONTRATO_FIRMA, UserFileType.CONTRATO_FIRMA_ADELANTO_SUELDO);

                    UserFile userFile = userFiles.stream().findFirst().orElse(null);

                    String url = "/file/splitUserFile/" + fileService.generateUserFileEncrypt(userFile.getUserId(), userFile.getFileName(), userFile.getId()) + "/image.jpg";

                    if (Configuration.hostEnvIsLocal())
                        url = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + "/backoffice" + url;
                    else
