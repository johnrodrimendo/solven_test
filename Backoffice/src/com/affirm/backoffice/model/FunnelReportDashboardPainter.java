...
import com.affirm.common.model.FunnelDashboardSection;
import com.affirm.common.model.RowGoogleAnalytics;
import com.affirm.common.model.RowGoogleAnalyticsDetail;
...
public class FunnelReportDashboardPainter {
    ...
    private List<BotsReportPeriod> botsPeriods;
    ...
    private void processBotsReport() throws Exception {
        mapBots = new HashMap<>();

        botRows = new ArrayList<>();
        periods = new ArrayList<>();
        for (BotsReportPeriod botPeriod : botsPeriods) {
            periods.add(botPeriod.getPeriod());
            for (BotsReportPeriodDetail detail : botPeriod.getDetails()) {
                RowBot row = mapBots.get(detail.getBotId());
                if (row == null) {
                    row = new RowBot();
                    row.setName(detail.getBot().getName());
                    row.setBotId(detail.getBotId());
                    mapBots.put(detail.getBotId(), row);
                    botRows.add(row);
                }
                RowBotDetail rowBotDetail = new RowBotDetail();

                rowBotDetail.setCost(detail.getCost());
                rowBotDetail.setQuantity(detail.getQuantity());

                if (row.getValues() == null) {
                    row.setValues(new HashMap<>());
                }
                row.getValues().put(botPeriod.getPeriod(), rowBotDetail);
            }
        }
    }
        ...
        for (FunnelDashboardSection section : sections) {
            ...
            for (String period : periods) {
                ...
                for (String row : section.getRows()) {
                    ...
                    if (perVisit != null && !perVisit.equals("") && !section.getName().equals("Consumos variables")) {
//                        if(visits.get(period) != 0) {
//                            values.set(1, utilService.percentFormat( 100.0 * Integer.valueOf(perVisit) / visits.get(period),"PE"));
//                        }
//                        else {
//                            values.set(1, utilService.percentFormat(0.0, "PE"));
//                        }i
    ...
    public List<BotsReportPeriod> getBotsPeriods() {
        return botsPeriods;
    }

    public void setBotsPeriods(List<BotsReportPeriod> botsPeriods) {
        this.botsPeriods = botsPeriods;
    }
